...
function makeConstructorFinisher(parser) {
    return function(e) {
        ...
        function copyProperty (propertyName) {
          var src = doclet[propertyName]
          var dest = parentDoclet[propertyName]
          if (!dest && src) {
            parentDoclet[propertyName] = Array.isArray(doclet[propertyName])
                ? doclet[propertyName].slice(0)
                : doclet[propertyName]
          }
        }

        propertiesToCopy.forEach(copyProperty)
