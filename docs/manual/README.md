# SmilesDrawer Manual

## Basic Usage

- [Basic Drawing](basic-drawing.md)\
  Draw some molecules.

- [Customization](customization.md)\
  Fine-tune your drawings and use custom colors.

- [Drawing Reactions](reactions.md)\
  Draw chemical reactions.

## Advanced Topics

- [Frameworks](../frameworks/README.md)\
  Use SmilesDrawer inside other JavaScript frameworks.

- [Atom Weights](atom-weights.md)\
  Make your molecules glow green and/or red.

- [Reaction Weights](reaction-weights.md)\
  Apply weights to atoms in reactions.

- [Atom Highlighting](atom-highlighting.md)\
  An older alternative to atom weights.

- [Entrypoints](entrypoints.md)\
  An advanced guide to the many entrypoints of SmilesDrawer.


## Tools

- [Molecule Viewer](https://reymond-group.github.io/smilesDrawer/tools/viewer.html)
- [Multi-Molecule Viewer](https://reymond-group.github.io/smilesDrawer/tools/multi-viewer.html)
- [Theme Gallery](https://reymond-group.github.io/smilesDrawer/tools/theme-gallery.html)
- [The Playground](https://reymond-group.github.io/smilesDrawer/tools/playground.html)


## Miscellaneous

- [Homepage](https://reymond-group.github.io/smilesDrawer)
- [GitHub Repo](https://github.com/reymond-group/smilesDrawer)
